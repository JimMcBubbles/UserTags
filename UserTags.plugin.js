	...
	changelog: [
		{
			title: "2026-02-05b",
			items: [
				"Restored sort selector and added Oldest/Newest DM message columns to the overview grid.",
				"Kept DM-on-view logging and refreshed cached bounds when changing viewed DM channels."
			]
		},
		{
			title: "2026-02-05",
...
const getDmMessageBounds = (channelId, options = {}) => {
        const { forceRefresh = false } = options;
        if (!channelId) return { oldest: null, newest: null, totalMessages: 0 };

        if (!forceRefresh && dmMessageBoundsCache.has(channelId)) {
                return dmMessageBoundsCache.get(channelId);
        }

        const MessageStore = getMessageStore();
        if (!MessageStore) return { oldest: null, newest: null, totalMessages: 0 };
        ...
        dmMessageBoundsCache.set(channelId, bounds);
        return bounds;
};

const dmMessageBoundsCache = new Map();

const USER_COL_KEY = "__USER__";
const OLDEST_MSG_COL_KEY = "__OLDEST_MSG__";
const NEWEST_MSG_COL_KEY = "__NEWEST_MSG__";
	...
	getAllTags() {
        ...
        startDmViewObserver() {
                ...
                const logCurrentDmBounds = () => {
                        ...
                        this.lastViewedDmChannelId = channelId;
                        const bounds = getDmMessageBounds(channelId, { forceRefresh: true });
		...
		function Panel() {
			...
			const totalAssignments = baseUsers.reduce(
				...
				0
                        ...
                        const ChannelStore = getChannelStore();
                        const dmBoundsByUser = new Map();
                        const getBoundsForUser = (userId) => {
                                if (dmBoundsByUser.has(userId)) return dmBoundsByUser.get(userId);

                                const channelId = ChannelStore?.getDMFromUserId?.(userId) || null;
                                const bounds = channelId
                                        ? getDmMessageBounds(channelId)
                                        : { oldest: null, newest: null, totalMessages: 0 };

                                dmBoundsByUser.set(userId, { channelId, ...bounds });
                                return dmBoundsByUser.get(userId);
                        };
                        ...
                        if (sortMode === "alpha") {
                                ...
                                sortedUsers.sort((a, b) => {
                                        const aOldest = getBoundsForUser(a.userId).oldest?.timestamp ?? null;
                                        const bOldest = getBoundsForUser(b.userId).oldest?.timestamp ?? null;

                                        if (aOldest == null && bOldest == null) return alphaCompare(a, b);
                                        if (aOldest == null) return 1;
                                        if (bOldest == null) return -1;
                                        if (aOldest === bOldest) return alphaCompare(a, b);
                                        return aOldest - bOldest;
                                });
                        } else if (sortMode === "newest") {
                                sortedUsers.sort((a, b) => {
                                        const aNewest = getBoundsForUser(a.userId).newest?.timestamp ?? null;
                                        const bNewest = getBoundsForUser(b.userId).newest?.timestamp ?? null;

                                        if (aNewest == null && bNewest == null) return alphaCompare(a, b);
                                        if (aNewest == null) return 1;
                                        if (bNewest == null) return -1;
                                        if (aNewest === bNewest) return alphaCompare(a, b);
						...
						React.createElement(
							...
							React.createElement(
								...
								React.createElement(
									...
									React.createElement("option", { value: "oldest" }, "Oldest message"),
									React.createElement("option", { value: "newest" }, "Newest message")
								)
							),
							React.createElement(
								"button",
			...
			// Build grid template columns based on current widths
                        const columnWidths = [
                                ...
                                colWidths[OLDEST_MSG_COL_KEY] || 140,
                                colWidths[NEWEST_MSG_COL_KEY] || 140,
                        ...
                        gridChildren.push(
                                React.createElement(
                                        ...
                                        React.createElement(
                                                "div",
                                                { className: "usertags-header-cell" },
                                                React.createElement("span", { className: "usertags-header-label" }, "Oldest"),
                                                React.createElement("span", {
                                                        className: "usertags-col-resizer",
                                                        onMouseDown: (e) => startResize(e, OLDEST_MSG_COL_KEY)
                                                })
                                        )
                                )
                        );
                        ...
                        gridChildren.push(
                                React.createElement(
                                        ...
                                        React.createElement(
                                                "div",
                                                { className: "usertags-header-cell" },
                                                React.createElement("span", { className: "usertags-header-label" }, "Newest"),
                                                React.createElement("span", {
                                                        className: "usertags-col-resizer",
                                                        onMouseDown: (e) => startResize(e, NEWEST_MSG_COL_KEY)
                                                })
                                        )
                                )
                        );
					...
					(hoverUserId === user.userId ? " usertags-row-hover" : "");
						...
						React.createElement(
							...
							user.avatarUrl
                                ...
                                const bounds = getBoundsForUser(user.userId);
                                const oldestTs = bounds.oldest?.timestamp ?? null;
                                const newestTs = bounds.newest?.timestamp ?? null;

                                const oldestText = oldestTs != null ? new Date(oldestTs).toLocaleDateString() : "—";
                                const newestText = newestTs != null ? new Date(newestTs).toLocaleDateString() : "—";

                                const oldestTitle = oldestTs != null
                                        ? `Oldest message: ${new Date(oldestTs).toLocaleString()}`
                                        : "No DM messages found";
                                const newestTitle = newestTs != null
                                        ? `Newest message: ${new Date(newestTs).toLocaleString()}`
                                        : "No DM messages found";
